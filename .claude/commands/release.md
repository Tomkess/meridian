...
3. Run it for real:
   ...
   ```
   The script bumps `meridian/__init__.py` (the single source of the version —
   `pyproject.toml` reads it dynamically), runs pytest + ruff + mypy, prepends a
   CHANGELOG section generated from commits since the last tag, commits, tags,
