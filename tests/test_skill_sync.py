"""Drift guard: bundled skills ↔ in-repo ``.claude/commands`` copies (FEAT-003).

Meridian ships each skill twice — the source of truth in
``meridian/skills/commands/`` and the in-repo Claude Code copies in
``.claude/commands/`` that this repository uses when dogfooding. They must stay
...
INREPO = REPO_ROOT / ".claude" / "commands"
...
def test_same_skill_set() -> None:
    ...
    assert not missing and not extra, (
        f"Skill set drift between bundled and .claude/commands.\n"
        f"  Missing from .claude/commands: {sorted(missing)}\n"
        f"  Extra in .claude/commands:     {sorted(extra)}\n"
        f"Re-sync .claude/commands/ with meridian/skills/commands/."
    )
...
def test_bundled_and_inrepo_are_structurally_consistent(skill_path: Path) -> None:
    ...
    if not counterpart.exists():
        pytest.skip(f"{skill_path.name} has no .claude/commands counterpart")
    ...
    assert report.consistent, (
        ...
        f"its .claude/commands copy:\n"
        + report.render("bundled", ".claude/commands")
    )
