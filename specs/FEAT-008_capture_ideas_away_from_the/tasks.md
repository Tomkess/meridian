...
- [ ] 1. Add `meridian/home.py` with `meridian_home() -> Path` (honours `MERIDIAN_HOME`, defaults
       ...
       Pre: none
       AC: #1, #5
- [ ] 2. Write `tests/test_home.py`: `MERIDIAN_HOME` override respected, directories created on
...
- [ ] 3. Add `meridian/inbox.py` with the `Capture` dataclass and `read_capture(path) -> Capture`:
       ...
       Pre: task 1 complete
       AC: #2, #3, #4
- [ ] 4. Add `write_capture(text, *, project=None) -> Path` and `list_captures() -> list[Capture]`
       ...
       Pre: task 3 complete
       AC: #1
- [ ] 5. Write `tests/test_inbox.py` covering: frontmatter present / absent / malformed, bare-text
...
- [ ] 6. Add `meridian capture "<text>"` to `cli.py`. Must not call `_config()` — it has to work
       from any directory on the machine, including outside a repo. Print the written path.
       Pre: task 4 complete
       AC: #5
- [ ] 7. Write CLI tests for `capture`, including the case that matters most: run it from a
...
- [ ] 8. Add `meridian/registry.py`: `ProjectEntry` dataclass, `all_projects()` reading
       ...
       Pre: task 1 complete
       AC: #6, #7, #8
- [ ] 9. Write `tests/test_registry.py`: register creates, re-register updates in place (no
       duplicate), stale path flagged `exists=False` and retained, missing file, malformed TOML.
       Pre: task 8 complete
       AC: #7, #8
- [ ] 10. Wire registration into `meridian init` and `meridian install`, keyed by the FEAT-007
       `cfg.project` slug. Purpose line comes from `specs/VISION.md` first heading if present,
       else the repo directory name.
       Pre: task 8 complete
       AC: #7
- [ ] 11. Add `meridian projects` listing slug, path, and resolve status.
       Pre: task 8 complete
       AC: #9

### 5 — Triage

- [ ] 12. Add `meridian inbox` listing pending captures: index, capture date, first line, and the
       routing hint where one exists (shown as `explicit`, not a score). No suggestions yet.
       Pre: tasks 4 and 8 complete
       AC: #10, #13
- [ ] 13. Add `meridian inbox route <id> --project <slug>`: resolve the target repo from the
       ...
       AC: #14, #15
- [ ] 14. Make `route` idempotent and safe: a already-processed id fails with a clear message and
       ...
       AC: #17, #19, #20
- [ ] 15. Add `meridian inbox drop <id>` moving the capture to `.icebox/`. Never deletes.
       Pre: task 12 complete
       AC: #16
- [ ] 16. Write triage tests: full round trip (capture → list → route → spec exists in target repo,
       ...
       AC: #14, #16, #17
- [ ] 17. Confirm no auto-file path exists: add a test asserting there is no CLI flag or code path
...
- [ ] 18. Add `meridian/routing.py`: `suggest(capture, entries) -> list[Suggestion]`. Explicit hint
       ...
       AC: #11, #13
- [ ] 19. Add the `purpose` fallback: projects with no rows in the shared index are scored against
       their registry `purpose` text, so a brand-new project is never permanently unroutable.
       Pre: task 18 complete
       AC: #12
- [ ] 20. Write `tests/test_routing.py` with a stubbed `search_similar`: score aggregation across
       ...
       AC: #11, #12, #13
- [ ] 21. Wire suggestions into `meridian inbox`, showing three candidates with scores and their
...
- [ ] 22. Show the pending inbox count in `meridian status`. [DECISION NEEDED: this puts global
       state into a per-project dashboard — spec Open Questions leans yes on the grounds that an
       invisible inbox is an unemptied inbox. Confirm before building.]
       Pre: task 4 complete
- [ ] 23. Document `capture`, `inbox`, and `projects` in `meridian help` and `CLAUDE.md`, including
...
- [ ] 24. Add a STEERING.md note: tests must set `MERIDIAN_HOME` to a temp path and never touch the
       real `~/.meridian`. The FEAT-007 wipe is the precedent worth naming.
       Pre: task 1 complete
- [ ] 25. Manual verification: capture from a scratch directory outside any repo, triage in this
